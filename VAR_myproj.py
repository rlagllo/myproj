...
baseFlpth = "https://m-selig.ae.illinois.edu/ads/"		# 고정되는 인터넷 주소?			# Base filepath for saving
saving_loc = "C:\\Users\\김해창\\Desktop\\airfoildata"
...
for link in soup.find_all('a',attrs={'href': re.compile('\.dat', re.IGNORECASE)}):      # Loop over all appropriate links on webpage, IGNORECASE: dat & DAT 전부 끌어옴
    #links.append(link.get('href'))													# Append the link to the list
    
    urllib2.urlretrieve(baseFlpth+link.get('href'),os.path.join(saving_loc, link.get('href').rsplit('/',1)[-1]))	# Get the data from the webpage, and save it to the save data file as the link name
