...
for i, image_path in enumerate(image_files):
    ...
    try:
        ...
        angles = np.linspace(0, np.pi, 100)  # 0에서 180도(π 라디안)까지 100 단계로 나눔
        positions = []
        ...
        stl_mesh = mesh.Mesh(mesh_data)
        stl_mesh.save(f'output_b_merg{now_img_it+1}.stl')

        print("STL 파일이 'output_b_merg.stl' 저장")
        # 기존 메쉬 로드 (Open3D 형식)
        vertices_file = 'merged_vertices.csv'
        faces_file = 'merged_faces.csv'

        # CSV에서 정점 및 면 데이터 로드
        vertices = np.loadtxt(vertices_file, delimiter=',', skiprows=1)
        faces = np.loadtxt(faces_file, delimiter=',', skiprows=1, dtype=int)

        # 각 면에 대해 면의 정점 개수와 인덱스를 결합
        faces_pv = []
        for face in faces:
            faces_pv.append([3] + list(face))  # 삼각형 면의 경우 [3, v1, v2, v3]
