...
"""
def value_it(policy, states_size, action_size, gamma, theta): # gamma: discount factor, theta: threshold
    ...
    for state in range(states_size):
        ...
        for act in range(action_size):
            ...
            for transition in env.P[state][act]:
                ...
                Qval[act] += p * (reward + gamma * V[sprime])
...
new_policy = value_it(policy,states_size,action_size,gamma,theta)
while not done:
